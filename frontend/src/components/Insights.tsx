...
type Props = { insight: Insight; unavailableMessageIds?: string[]; onLocateMessage: (messageId: string) => void };
...
export default function Insights({ insight, unavailableMessageIds = [], onLocateMessage }: Props) {
  return (
    <section className="insights-panel" aria-label="讨论洞察">
      ...
      <div className="insight-list">
        ...
        {unavailableMessageIds.length > 0 && <div className="insight-group"><h3><span>05</span>未完成审查<em>{unavailableMessageIds.length}</em></h3>{unavailableMessageIds.map((messageId) => <div className="insight-card" key={messageId}><p>这一轮未能完成事实风险审查，不能据此认定发言无风险。</p><a href={`#message-${messageId}`} onClick={() => onLocateMessage(messageId)}>查看未审查发言 ↗</a></div>)}</div>}

      ...
      <main className="studio-main">
        <div className="studio-title-row">
          ...
          <div className="session-controls">
            ...
            {snapshot.status === 'failed' && <button onClick={() => void handleAction('resume')} disabled={busy}>继续讨论 <span aria-hidden="true">↗</span></button>}
        ...
        {snapshot.status === 'failed' && <div className="studio-banner error-banner">讨论中断了，已保留之前的发言和洞察。点击“继续讨论”从上次检查点恢复。{snapshot.error_code && <span>错误代码：{snapshot.error_code}</span>}</div>}
        ...
        <div className="studio-grid">
          ...
          <div className={`studio-grid-cell insights-cell ${activeTab === 'insights' ? 'mobile-active' : ''}`}><Insights insight={snapshot.insight} onLocateMessage={locateMessage} /></div>
        </div>

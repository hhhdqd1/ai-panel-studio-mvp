...
完成的判断标准：用户输入任意议题并指定专家人数后，可确认 AI 生成的阵容，观看主持人与专家进行非固定顺序的讨论；页面实时显示发言、公开 Agent 状态、共识、分歧与事实风险提示；结束时呈现自然语言总结。两场讨论可并行运行且数据不串台。项目有测试、五条高质量示例、Prompt 记录和清晰的 Git 历史。
...
- 实时展示：专家席位状态、公开意图摘要、讨论记录、持续更新的共识/分歧/待追问点，以及对可疑事实断言的“待核实”提示。
...
桌面端演播厅有三个各自滚动的区域：圆桌席位、实时讨论记录、讨论洞察。圆桌是主要视觉焦点，主持人和专家围桌就座，桌心显示当前问题；正在发言的席位使用整块高对比色、光晕和文字状态，举手席位有独立标识，记录区域同步突出当前发言。讨论洞察区包含共识、分歧、待追问点和待核实断言；每条提示链接到对应发言并说明触发原因。只展示模型生成的可公开意图摘要，例如“准备补充课堂案例”，绝不展示或保存模型隐藏推理内容。窄屏通过“圆桌 / 记录 / 洞察”标签切换当前区域，保持区域自身滚动。结束后显示自然语言总结和未解决的事实风险提示，不呈现原始 JSON。
...
| `Insight` | `id`, `discussion_id`, `version`, `consensus`, `disagreements`, `open_questions`, `claim_flags` | 洞察与事实风险版本快照；每条项目内含来源消息 ID |
...
事件包括 `discussion.status`、`panel.ready`、`agent.updated`、`message.created`、`insight.updated`、`discussion.completed` 和 `discussion.failed`。`insight.updated` 同时携带最新的共识、分歧和事实风险提示。客户端先读取详情快照，再从 `last_event_seq` 之后建立 SSE 连接，并按事件序号去重。事件只在数据库提交成功后推送。
...
5. 选中专家生成 1～2 句发言，经长度和格式校验后持久化；状态变化与消息分别形成事件。每条专家发言后调用一次独立审查，更新洞察与事实风险提示；审查失败不阻塞讨论，页面显示“本轮审查暂不可用”。主持人根据重复度、未答问题、风险提示及阶段目标决定追问或推进。
...
审查器从新发言中提取具体、可核查的断言，优先检查数字、日期、具名研究、引文和明确的事实判断。它将断言与本场已保存的发言比对，标出“与讨论记录冲突”或“缺少可追溯依据”；需要外部资料才能判断的断言标为“待外部核实”。每条 `claim_flag` 保存来源 `message_id`、原句摘录、原因码、简短说明和 `open/clarified` 状态。`clarified` 仅表示专家作了进一步说明，不等于事实得到证实。只把具体且影响讨论结论的风险展示在页面上，同一断言去重；普通立场或价值判断不作为事实错误处理。

本阶段没有外部可信检索源，因此审查器不输出“真实/已核实”的结论，也不把另一轮模型回答当成证据。主持人可对高风险断言追问来源或适用条件；每阶段最多一次此类追问，避免讨论跑题。最终总结将未解决的断言列为“待核实”，不得改写成已证实的事实。未来若增加外部检索，可在保持相同 `claim_flag` 结构的前提下接入带来源链接的核验结果。
...
- 幻觉风险审查失败：保留发言及已有洞察，标记该轮“审查暂不可用”；继续讨论，不把未审查误标为安全。
...
- 单元测试：状态转换、Speaker Selector 非固定轮流且有公平性、结构化输出校验、洞察来源校验；编造引文、无依据数字和前后矛盾会得到待核实/冲突标记，普通意见不会被误判为事实。
- API/集成测试：创建 → 阵容确认 → 讨论 → 洞察 → 总结；失败与继续；SSE 补发；两场讨论交错运行但数据隔离。
- 浏览器 E2E：完成一场讨论、刷新后恢复 UI、圆桌状态与 transcript 同步、风险提示可定位原发言、窄屏切换和独立滚动、总结页面无原始 JSON 或隐藏推理。

...
GitHub 公开仓库：<https://github.com/hhhdqd1/ai-panel-studio-mvp>

> 交付定位：远程作业 MVP。事实风险提示不是联网事实核查，也不代表自动判定真伪。

## 已完成能力

- **议题与阵容确认**：支持输入 1–500 字自定义议题，或从五条预设议题开始；可选择 2–6 位专家，并在开场前查看主持人和专家的身份、立场与专长。
- **自主圆桌讨论**：专家根据当前上下文提出举手意愿，系统综合议题相关性、观点新颖度和发言公平性选择发言者；主持人负责开场、追问、转场和自然语言总结，不按固定顺序机械轮流。
- **实时展示与恢复**：圆桌席位、逐条发言和洞察通过 SSE 更新；SQLite 保存讨论及事件，刷新或短暂断线后可从快照与事件序号继续，失败场次可从检查点恢复，多场讨论互不混淆。
- **可追溯的风险提示**：展示共识、分歧、待追问和待核实断言；每条风险都能定位到真实发言原句。当前只提示证据风险，不对外部事实自动判真伪。
- **桌面与手机适配**：桌面端并列展示圆桌、记录和洞察；窄屏可用标签切换，并突出当前发言人。后端支持显式假模型演示与真实 DeepSeek 调用，API Key 不进入浏览器。
...
以下是本次 MVP 尚未完成的方向，不应视为现有功能：

- **接入外部证据**：为待核实断言提供可追溯的资料来源和引用链接，并保留人工复核环节，逐步从“风险提示”走向有依据的事实核查。
- **增加用户与分享能力**：加入账号、权限控制、讨论分享和导出，让多位用户能够安全地保存和交流讨论结果。
- **完善生产部署**：将编排任务和实时通知移出单个后端进程，支持多 worker、后台任务恢复及运行监控。
- **细化模型成本管理**：记录每次调用的 token 用量和费用，结合现有调用次数上限提供更清晰的预算与性能统计。
